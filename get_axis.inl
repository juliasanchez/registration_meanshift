void get_axis(std::vector<Cluster> clusters, std::vector< std::vector <float> >& axis, bool* error )
{
    *error=false;
    ...
    {
    double dot=norm1[0]*clusters[i].mode[0]+norm1[1]*clusters[i].mode[1]+norm1[2]*clusters[i].mode[2];
    ...
    else if(clusters[i].original_points.size()>temp2 && abs(dot)<0.90 && abs(clusters[i].mode[2])<0.7 )
    {
